...
export class Web3Service {
  ...
  // Chama a função getHotelAddress do contrato
  async getHotelAddress(bookingContractAddress: string): Promise<string> {
    try {
      const contract = new this.web3.eth.Contract(this.contractABIBooking, bookingContractAddress);
      return await contract.methods.getHotelAddress().call();
    } catch (error) {
      console.error('Erro ao obter o endereco do hotel:', error);
       throw error;
    }
  }
